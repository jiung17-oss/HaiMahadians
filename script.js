...
/* ─── PRELOADER ─────────────────────────────── */
// Called AFTER all functions are defined — no hoisting issues
function initPreloader() {
const bar   = $(‘preBar’);
const count = $(‘preCount’);
const pre   = $(‘preloader’);

if (!pre) return; // Safety: if element missing, skip entirely

document.body.classList.add(‘loading’);

let pct = 0;

// Hard fallback: force-dismiss after 4s no matter what
const hardFallback = setTimeout(() => dismissPreloader(pre), 4000);
...
setTimeout(() => dismissPreloader(pre), 350);
return;
}
...
setTimeout(() => {
pre.style.display = ‘none’; // Completely remove from layout
